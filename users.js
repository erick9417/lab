...
router.get("/", async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      "SELECT u.id, u.name, u.email, u.role, u.clinic_id, u.disabled, c.name as clinic_name FROM users u LEFT JOIN clinics c ON u.clinic_id = c.id ORDER BY u.email LIMIT ? OFFSET ?",
      [limit, offset]
    )
    res.json(rows)
  } catch (e) {
...
router.post("/", async (req, res) => {
  const { email, password, name, role, clinic_id, clinicId } = req.body
  ...
  try {
    ...
    const [result] = await pool.query(
      ...
      [email, password_hash, name || null, role, finalClinicId ?? null]
    )
    res.status(201).json({ id: result.insertId, email, name: name || null, role, clinic_id: finalClinicId ?? null })
  } catch (e) {
    console.error(e)
...
router.put("/:id", async (req, res) => {
  const id = parseInt(req.params.id)
  const { email, name, role, clinic_id, clinicId } = req.body
  const finalClinicId = clinic_id !== undefined ? clinic_id : clinicId
  ...
  try {
    ...
    return res.json({ id, email, name: name || null, role, clinic_id: finalClinicId ?? null })
  } catch (e) {
    console.error(e)
